...
namespace moonlight
{
    public partial class MOL_ChooseGame_2 : Form
    {
        private static int iCountQ1 = 0;
        public MOL_ChooseGame_2()
        {
            InitializeComponent();
            DrawNew();
        }
        private void DrawNew()
        {
            using (var context = new moreorlessEntities())
            {
                ...
                lbl_MOL_Cost_1.Text = context.Champions.Where(x => x.ChampionName == lbl_MOL_ChampionName_1.Text).Select(x => x.ChampionCost).SingleOrDefault().ToString();
                Bitmap bmp;
                Bitmap bmp2;
                ...
                lbl_MOL_Cost_2.Text = context.Champions.Where(x => x.ChampionName == lbl_MOL_ChampionName_2.Text).Select(x => x.ChampionCost).SingleOrDefault().ToString();
        ...
        {
            btn_MOL_Less_Costs.Visible = false;
            btn_MOL_More_Costs.Visible = false;
            lbl_MOL_Cost_2.Visible = true;
        ...
        private void btn_MOL_More_Costs_Click(object sender, EventArgs e)
        {
            if (int.Parse(lbl_MOL_Cost_2.Text) > int.Parse(lbl_MOL_Cost_1.Text) || int.Parse(lbl_MOL_Cost_2.Text) == int.Parse(lbl_MOL_Cost_1.Text))
            {
                AddScore();
                DrawNew();
            }
        ...
        private void btn_MOL_Less_Costs_Click(object sender, EventArgs e)
        {
            if (int.Parse(lbl_MOL_Cost_2.Text) < int.Parse(lbl_MOL_Cost_1.Text) || int.Parse(lbl_MOL_Cost_2.Text) == int.Parse(lbl_MOL_Cost_1.Text))
            {
                AddScore();
        ...
        private void btn_MOL_Submit_Game_1_Score_Click(object sender, EventArgs e)
        {
            if (tb_MOL_PlayerName.Text == "")
                ...
                using (var context = new moreorlessEntities())
                {
                    context.ScoreCostAdd(tb_MOL_PlayerName.Text, iCountQ1);
                    MessageBox.Show("Score added to ranking!");
                    ClearAll();
                    DrawNew();
                    btn_MOL_Less_Costs.Visible = true;
                    btn_MOL_More_Costs.Visible = true;
                }
        }
